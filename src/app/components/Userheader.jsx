...
import { Container, Navbar, Nav, Image, NavDropdown, Button, Tab, Table, Form, Modal } from 'react-bootstrap';
import { usePathname } from 'next/navigation'
import '../../../public//assets/css/user.css';
import SimpleBar from 'simplebar-react';
import 'simplebar-react/dist/simplebar.min.css';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCheck, faCheckCircle, faEye, faCopy, faArrowRight, faEyeSlash } from '@fortawesome/free-solid-svg-icons';
...
const Userheader = () => {
  ...
  const [selected, setSelected] = useState({
    label: "USDC",
    img: "/assets/images/color/usdc.svg"
  });

  const options = [
    { label: "Bitcoin", img: "/assets/images/color/btc.svg", value: "btc" },
    { label: "Ethereum", img: "/assets/images/color/eth.svg", value: "eth" },
    { label: "Solana", img: "/assets/images/color/sol.svg", value: "sol" },
  ];
  ...
  {/* <Offersection /> */}
    ...
    <header className="headermenu headermenu-user">
        ...
        <Container>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="navbar-nav leftheadbg">
              ...
              <HoverDropdown title="Trade" id="trade">
                <NavDropdown.Item as={Link} href="/trade" className={`${pathname === '/trade' ? 'active' : ''}`}>
                  <div className="tableicon"><span><span className="iconboxbg"><Image src="/assets/images/spoticon.svg" alt="icon" width={100} height={100} />
                  </span></span> <span>Spot<br /><small className="t-gray">Buy and Sell crypto with ease</small></span></div>
                </NavDropdown.Item>
                <NavDropdown.Item as={Link} href="/perpetual" className={`${pathname === '/perpetual' ? 'active' : ''}`}>
                  <div className="tableicon"><span><span className="iconboxbg"><Image src="/assets/images/perp.svg" alt="icon" width={100} height={100} /></span>
                  </span> <span>Perpetual<br /><small className="t-gray">Trade perpetual futures with leverage</small></span></div>
                </NavDropdown.Item>
                <NavDropdown.Item as={Link} href="/trade" className={`${pathname === '/trade' ? 'active' : ''}`}>
                  <div className="tableicon"><span><span className="iconboxbg"><Image src="/assets/images/perp-dex.svg" alt="icon" width={100} height={100} />
                  </span></span> <span>Perpetual - <span className="navbar-tt">Dex</span><br /><small className="t-gray">Go long or short with perpetual DEX</small>
                  </span></div>
                </NavDropdown.Item>
                {/* <NavDropdown.Item as={Link} href="/trade" className={`${pathname === '/trade' ? 'active' : ''}`}>
                  <div className="tableicon"><span>Spot</span></div>
                </NavDropdown.Item>
                <NavDropdown.Item as={Link} href="/trade" className={`${pathname === '/perpetual' ? 'active' : ''}`}>
              ...
              {/* <Nav.Item>
                <Link
                  href="/perpetual"
                  className={`nav-link ${pathname === '/perpetual' ? 'active' : ''}`}
                  id="perpetuallink"
                >
                  Perpetual
                </Link>
              ...
              <Nav.Item>
                <Link
                  ...
                  id="perpetuallink"
                >
                  Perpetual - <span className="navbar-tt">Dex</span>
                </Link>
              </Nav.Item>
            ...
            <Nav className="navbar-nav ms-auto rightheadbg">
              ...
              {/* <Nav.Item className="web-connect">
                <Button className="nav-link btn sitebtn ms-2" id="depositlink" onClick={handleShowModal1}>
                  Connect Wallet
                </Button>
              </Nav.Item> */}
              <li className="nav-item cartmenuicon dropdown" >
                <Link className="nav-link cartmenulink dropdown-bs-toggle" href="#" data-bs-toggle="dropdown" data-bs-auto-close="outside">
                  <span className="photopic"><Image src="assets/images/profile.svg" alt="profile" /></span>
                </Link>

                {/* MAIN BOOTSTRAP DROPDOWN */}
                <div className="dropdown-menu">
                  <div className="profilesubdropbox profilesubdropbox-x p-2">
                    <div className="balanceshowt totblance pb-2 gradientdarkbg">
                      <span className="photopic">
                        <Image src="assets/images/profile.svg" alt="icon" width={100} height={100} id="profile" />
                      </span>
                      <h5>Michael <span className="t-green"> <FontAwesomeIcon icon={faCheckCircle} /></span> <br /><span className="t-gray">Xx.5120xs1... <FontAwesomeIcon icon={faCopy} /></span></h5>
                      <h4 className="h4">*.******** USDC <span className="ps-2"><FontAwesomeIcon icon={faEyeSlash} /></span></h4>
                    </div>
                  </div>
                  <div className="w-100 mb-2 px-2 mo-connect">
                    <Button className="btn sitebtn  w-100" id="depositlink" onClick={handleShowModal1}>
                      Connect Wallet
                    </Button>
                  </div>

                  {/* CUSTOM SELECT SECTION */}
                  <div className="pro-drop-select-container" onClick={(e) => e.stopPropagation()}>
                    <div
                      className={`pro-drop-selected ${isOpen ? 'active' : ''}`}
                      onClick={(e) => {
                        e.stopPropagation()
                        setIsOpen(!isOpen)
                      }}
                    >
                      <Image src={selected.img} alt="icon" width={20} height={20} />
                      <span>{selected.label}</span>
                      <span className="pro-drop-arrow">▾</span>
                    </div>

                    {isOpen && (
                      <ul className="pro-drop-options-list">
                        {options.map((opt) => (
                          <li
                            key={opt.value}
                            className="pro-drop-option-item"
                            onClick={() => {
                              setSelected(opt);
                              setIsOpen(false);
                            }}
                          >
                            <Image src={opt.img} alt={opt.label} width={20} height={20} />
                            {opt.label}
                          </li>
                        ))}
                      </ul>
      ...
      <Modal className="modalbgt cntmodal" show={showModal1} onHide={handleCloseModal1} centered>
        <Modal.Header closeButton>
          <Modal.Title>Connect Wallet</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {/* <Walletconnect /> */}
          <div className="connectwalletbg">
            <p>Select a wallet to connect to Digital Freemasonry.
              Once connected, you’ll be able to explore collections, create NFTs, and manage your assets.</p>
            <div className="d-flex gap-2 mt-2">
              <div>
                <Image src='assets/images/wallet-connect.svg' width={100} height={100} alt="wallet" /> <h4>Wallet Connect</h4>
              </div>
              <div>
                <Image src='assets/images/meta-mask.svg' width={100} height={100} alt="wallet" /> <h4>Meta Mask</h4>
              </div>
              <div>
                <Image src='assets/images/okx-wallet.svg' width={100} height={100} alt="wallet" /> <h4>OKX Wallet</h4>
              </div>
              <div>
                <Image src='assets/images/trust-wallet.svg' width={100} height={100} alt="wallet" /> <h4>Trust Wallet</h4>
              </div>
              <div>
                <Image src='assets/images/coinbase-wallet.svg' width={100} height={100} alt="wallet" /> <h4>Coinbase Wallet</h4>
              </div>
            </div>
          </div>
        </Modal.Body>
